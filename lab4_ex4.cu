...
int main(int argc, char **argv) {
  ...
  cusparseDnVecDescr_t tempDescr;
  cusparseDnVecDescr_t tmpDescr;
  cusparseCheck(cusparseCreateCsr(&spMatDescr, dimX, dimX, nzv, ARowPtr, AColIndx, A, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_32I, CUSPARSE_INDEX_BASE_ZERO, CUDA_R_64F));
  cusparseCheck(cusparseCreateDnVec(&tempDescr, dimX, temp, CUDA_R_64F));
  cusparseCheck(cusparseCreateDnVec(&tmpDescr, dimX, tmp, CUDA_R_64F));
  ...
  cusparseCheck(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, spMatDescr, tempDescr, &zero, tmpDescr, CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, &bufferSize));
  ...
  int timeSpent = -1;
  struct timeval t_start_spmv;
  struct timeval t_end_spmv;
  ...
  for (int it = 0; it < nsteps; ++it) {
    ...
    gettimeofday(&t_end_spmv, 0);
    if (timeSpent == -1) {
      timeSpent = 0;
    } else {
      timeSpent += (int)(1000000.0*(t_end_spmv.tv_sec-t_start_spmv.tv_sec) + t_end_spmv.tv_usec-t_start_spmv.tv_usec);
    }
  ...
  FILE *result_time=fopen("result_ex4-time.txt", "a");
  fprintf(result_time, "%d\n",timeSpent);
  printf("time spent: %d\n", timeSpent);
  ...
  FILE *pFile2=fopen("result_ex4.txt", "a");
  fprintf(pFile2, "%f\n",error);
  //@@ Insert the code to destroy the mat descriptor
  cusparseCheck(cusparseDestroySpMat(spMatDescr));
  cusparseCheck(cusparseDestroyDnVec(tempDescr));
  cusparseCheck(cusparseDestroyDnVec(tmpDescr));
